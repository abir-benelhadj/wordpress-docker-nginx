...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="post-main-box wow zoomInDown delay-1000" data-wow-duration="2s">
    ...
    if($moving_company_lite_theme_lay == 'Default'){ ?>
      <div class="row m-0">
        <?php if(has_post_thumbnail() && get_theme_mod( 'moving_company_lite_featured_image_hide_show',true) == 1) {?>
          <div class="box-image col-lg-6 col-md-6">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
        <div class="new-text <?php if(has_post_thumbnail() && get_theme_mod( 'moving_company_lite_featured_image_hide_show',true) == 1) { ?>col-lg-6 col-md-6"<?php } else { ?>col-lg-12 col-md-12" <?php } ?>>
          ...
          <?php if( get_theme_mod( 'moving_company_lite_toggle_postdate',true) == 1 || get_theme_mod( 'moving_company_lite_toggle_author',true) == 1 || get_theme_mod( 'moving_company_lite_toggle_comments',true) == 1 || get_theme_mod( 'moving_company_lite_toggle_time',true) == 1) { ?>
            <div class="post-info">
              <?php if(get_theme_mod('moving_company_lite_toggle_postdate',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('moving_company_lite_toggle_postdate_icon','fas fa-calendar-alt')); ?>"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $moving_company_lite_archive_year, $moving_company_lite_archive_month, $moving_company_lite_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span><?php echo esc_html(get_theme_mod('moving_company_lite_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('moving_company_lite_toggle_author',true)==1){ ?>
               <i class="<?php echo esc_attr(get_theme_mod('moving_company_lite_toggle_author_icon','far fa-user')); ?>"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span><span><?php echo esc_html(get_theme_mod('moving_company_lite_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('moving_company_lite_toggle_comments',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('moving_company_lite_toggle_comments_icon','fa fa-comments')); ?>" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'moving-company-lite'), __('0 Comments', 'moving-company-lite'), __('% Comments', 'moving-company-lite') ); ?> </span><span><?php echo esc_html(get_theme_mod('moving_company_lite_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('moving_company_lite_toggle_time',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('moving_company_lite_toggle_time_icon','fas fa-clock')); ?>"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
              <?php } ?>
    ...
    <?php }else if($moving_company_lite_theme_lay == 'Center'){ ?>
      <div class="service-text">
        <h2 class="section-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
        <?php if( get_theme_mod( 'moving_company_lite_featured_image_hide_show',true) == 1) { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
